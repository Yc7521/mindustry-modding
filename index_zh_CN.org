...
*** 分离机
    ...
    分离机是一个输入液体并以物品组中和数量为概率随机输出物品的机器,using the amount of items in the stack as probability. Separator can't accept items as input, as it will output all the items you put in it, regardless of what you put in ~results~英文:Separator
...
*** 物品炮塔
    ...
    |---------+----------------------+----------+-|
    | maxAmmo | int                  |      30 | 弹仓 |
    | ammo    | { 字符串: [[子弹类][子弹类]] } |         | String is the name of an [[Item]], which will be used to select the type of bullet which will be shot. |
...
** 武器
   ...
   |----------------+------------+---------+------------|
   |                |            |         | <10>       |
   | name           | string     |         |  武器名,used to fetch the sprite of the weapon          |
