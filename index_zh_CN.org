...
** Hjson
   ...
   *[v99 新特性]* /currently partially implimented, ~content/~ 中使用Hjson./

   现在Mindustry不是用的JSON格式, /(但是JSON格式依旧可以被解析)/ 它解析一个JSON的分支 [[https://hjson.org/][Hjson]].
...
** 方块
  ...
  | drawLiquidLight     | boolean         |         true | *[新特性 v99 (268)]* 在其中的液体是否发光. |
...
** 建筑菜单可见性
   ...
   * ~sandboxOnly~ 沙盒模式显示
   * ~campaignOnly~ 战役模式显示
   * ~lightingOnly~ 黑夜模式显示*[新特性 v99 (268)]*
...
** 液体
   ...
   | lightColor    | [[颜色][颜色]]        |         | *[new v99 (268)]* 发光颜色.改变光度不在这里. |
...
** 更新日志
   ...
   :DESCRIPTION: 日志
   :END:
   这个是Mindustry的主分支的更新日志.每个大版本用标签分开,比如 [[v100]] 大版本一下用hash值区分, 比如 [[968f3ace3]], 日志都带有世界. 这一些变更会在下一个大版本应用.

*** v100
**** 968f3ace3
     :PROPERTIES:
     :DESCRIPTION: 更好的mod解析
     :END:
     [2019-11-21 Thu]
     * ~"liquid/amount"~ 可以和 ~{ "liquid": "xxxx", "amount": xx }~ 在 [[液体组][液体组]] 中干同样的事;
     * ~"item/amount"~ 可以和 ~{ "item": "xxxx", "amount": xx }~ 在 [[物品组][物品组]] 中干同样的事;
     * ~mod.json~ 支持hjson;
     * ~mod.js~ 可以取代 ~mod.json~.
