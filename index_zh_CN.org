...
:PREAMBLE:
#+AUTHOR: Simon Woodbury-Forget Translater:PlasGensokyo
...
#+TEXINFO_HEADER: Modding documentation for Mindustry v102 (a4e820f90)
...
* 概览
  ...
  你可以添加自己的贴图和音效，打包上传并分享自己的mod，mod在所有平台上都是同一格式。如果你确实想用 [[GitHub][GitHub]], 你需要按照Anuken的格式布置: https://github.com/Anuken/ExampleMod
...
** 文件结构
   ...
   *Note;* 你的文件名必须全小写而且不能有空格:
   * 正确: ~my-custom-block.json~
   * 不正确: ~My Custom Block.json~
...
** Hjson
   ...
   *[new v99]* /在 v99 加入,但在 v100 工作/

   Mindustry 使用 [[https://hjson.org/][Hjson]], 一个JSON的分支.所以标准JSON格式也可以使用以，下是额外的东西:
   ...
   *[new #2d4270406]* 文件后缀可以是 =.json= 或 =.hjson= .
...
** ~mod.json~
   ...
   * *[new #2c61fcdfa]* ~minGameVersion~ 是主游戏最小支持版本.
...
** 内容
   ...
   * ~content/units/~ 用来放飞行或地面 [[单位类][单位类]], 比如 ~reaper~ 和 ~dagger~;
   * ~content/zones/~ 用来放 [[区域][区域]], 战役地图的配置文件.
   ...
   [ *new* #62b2b25ee474fcf44aa86832c1373b38e16d703d ] 在 ~content/<content-type>/*~ 目录下的文件可以放在子目录里面, 来帮助你分类, 例如:

   * ~content/items/metals/iron.hjson~, 这个创建了 ~iron~ 的物品.
...
** 科技树
   :PROPERTIES:
   :DESCRIPTION: 将方块加入科技树
   :END:

   像 ~type~ 一样的类，是 ~research~ 类，这个类也是根类.
   ...
   这个方块的研究会放在 ~duo~ 这个方块后面, 如果你想把某方块研究放在自己mod里面的一个方块后面，就写 ~<block-name>~, 跨mod调用需要在调用的方块前加那一个mod名字做前缀.

   研究消耗公式是 ~30 + requirements * 6~, ~requirements~ 是建筑耗材. /(换句话来说你不能在 ~requirements~ 中更改 ~research cost~ )/
...
** 贴图
   ...
   *[new #a5fbc0756]* 要覆盖原版材质, 你可以把材质放 ~sprites-override/~.
...
** 蓝图
   ...
   需要 ~Schematic~ 接口的类，不能替换原版发射图 /(详情请看 [[区域][区域]] 部分)/ 一个base64文字串,或者一个以 ~.msch~ 结尾的文件放在 ~schematics/~ 子目录里.
...
** 脚本
   ...
   *[new #268f3cc3c]*
   ...
   Mindustry的脚本依赖 [[https://github.com/mozilla/rhino][Rhino JavaScript]]. 你可以把脚本放在 ~scripts/~ 文件夹里面加入你的mod. 使用内建的 ~extendContent~ 方法, 你可以用JS继承Java类, 用 /allowed classes/ 来加入你的命名空间.
...
** GitHub
   ...
   你做了一些mod, 并且想发布它, 或者箱多人协力完成mod, 你可以选择使用 [[https://github.com/][GitHub]]. 如果你完全不知道什么是 Git (或 GitHub) , 你可以用 [[https://desktop.github.com/][GitHub Desktop]], 或者使用其他你喜欢的工具. 

   你只需要知道怎么在Github创建仓库, 进度的变更都在本地目录, 你需要将其推送到Github上. 如果你的项目在Github上, 这里有三个方法分享它:

   * 用仓库名, 比如 ~Anuken/ExampleMod~, 这样可以在游戏内拉取, 与下载;
   * 用压缩包, 比如 ~https://github.com/Anuken/ExampleMod/archive/master.zip~, 这里下载一个压缩包, 然后放在游戏mod目录 (无需解压);
   * 加上 ~mindustry-mod~ 这个tag在你的仓库, 这可以让 ~#mods频道~ 的Discord机器人识别并添加至mod列表.
...
** FAQ
    ...
    * <<Abstract>> 什么是 ~abstract~?关于此类你要知道的,只有这是Java一个修饰符,代表这个类不能直接被加载.如果你强行加载会出现 /"initialization exception"/ 诸如此类的错误;
    * 什么是 ~NullPointerException~?这是一个有一些类中的必须接口没有设置的信息;
    * <<bleeding-edge>> 什么是 ~bleeding-edge~? 这个是开发版的用Github上面的master分支构建.其中的新特性会在下一个正式版本中使用.
...
** 方块
  ...
  继承 [[方块存储][方块存储]]，英文:Block
  
  所有的物品和方块的基础类.所有方块至少要有一个贴图,通过识别方块名字加载.
  ...
  | rebuildable         | boolean         |         true | 这个能不能被建筑机重建 (比如 ~地雷~ 和 ~核电站~) |
  ...
  | sync                | boolean         |              | *[new a087df077 Dec 4] 是否sync这个方块. |
  | drawLiquidLight     | boolean         |         true | 在其中的液体是否发光. |
  ...
  | category            | [[内容菜单类型][内容菜单类型]]        | distribution | 建造菜单上面的种类. |
...
*** 消耗
    ...
    [[Abstract][Abstract]] 修饰类,英文:Consume
...
** 方块存储
   ...
   | outputsPower   | boolean   |   false | 是否输出能量 |
...
*** 地板
    ...
    | oreDefault        | boolean      |   false | 是否自然生成矿物在此地被上面.                               |
    | oreScale          | float        |      24 | 生成规模, for example ~thorium~ is ~25.38~, ~copper~ is ~23.47~. |
    | oreThreshold      | float        |   0.828 | 生存几率, for example ~thorium~ is ~0.882~, ~copper~ is ~0.81~.  |
...
*** 覆盖地板
    :PROPERTIES:
    :DESCRIPTION: 一种地板渲染在其他地板上一层
    :END:

    继承 [[地板][地板]],英文:OverlayFloor
...
*** 矿物
    :PROPERTIES:
    :DESCRIPTION:获取特定的物品所产生的矿物.
	:END:

    继承 [[覆盖地板][覆盖地板]],英文:OreBlock
...
*** 普通冶炼炉
    继承 [[普通加工厂][普通加工厂]],英文:GenericSmelter
...
*** 分离机
    :PROPERTIES:
    :DESCRIPTION: 将液体变成许多固体
    :END:
	
    继承 [[方块][方块]]

    分离机是一个输入液体并以物品组中和数量为概率随机输出物品的机器,用每个物品的数量来计算概率.分离机不接受物品作为原料, 它会把所有物品输出,不管你是否把它加入 ~results~英文:Separator
...
*** 门
    ...
    |---------+--------+-----------+-|
    |         |        |       <r> | |
    | openfx  | [[特效][特效]] |  dooropen | 开门特效 |
    | closefx | [[特效][特效]] | doorclose | 关门特效 |
...
** 炮塔s
   ...
   :DESCRIPTION: 射东西的方块
   :END:
   这一节是关于炮塔. 所以炮塔的子弹都是 [[子弹类][子弹类]], 这代表 [[液体炮塔][液体炮塔]] 可以使用 [[导弹][导弹]] 和 [[物品炮塔][物品炮塔]] 可以使用 [[液体子弹][液体子弹]].
...
*** 冷却炮塔
    ...
    继承 [[炮塔][炮塔]] -- 这是一个用 [[液体][液体]] 加速的炮塔的类.英文：CoolTurret
...
*** 物品炮塔
    ...
    继承 [[冷却炮塔][冷却炮塔]].英文：ItemTurret

	这个炮塔类用物品做子弹. ~ammo~ 的数据必须是 [[物品]], 具体值可以是 [[内建子弹]] 或自定义的 [[子弹类]].
    ...
    以上是用 ~copper~ 射出 ~standardCopper~ (内建子弹) 和用 ~metaglass~ 射击一个 ~MissileBulletType~ 子弹类型.

    | 接口   | 类型                 | 默认值 | 备注 |
    |---------+----------------------+----------+-|
    | maxAmmo | int                  |      30 | 弹仓 |
    | ammo    | { 字符串: [[子弹类][子弹类]] } |         | 字符串是 [[物品]] 的名字, 用来决定消耗的物品. |
...
*** 液体炮塔
    ...
    这个炮塔类用液体做子弹. ~ammo~ 的数据必须是 [[液体]], 具体值可以是 [[内建子弹]] 或自定义的 [[子弹类]].英文：LiquidTurret
...
*** 充能炮塔
    ...
    | chargeEffect      | [[特效][特效]] |    none | 充能特效 |
    | chargeBeginEffect | [[特效][特效]] |    none | 开始充能特效 |
...
*** 装甲传送带
    继承 [[传送带][传送带]]  -不接受两边来物从传送带，英文：ArmoredConveyor
...
*** 溢流门
    ...
    | 接口  | 类型    | 默认值 | 备注                                                                        |
    |--------+---------+---------+------------------------------------------------------------------------------|
    | speed  | float   | 1       | 物品通过时间 |
    | invert | boolean | false   | ~true~ 来反向                                                  |
...
*** 导管
    ...
    | 接口          | 类型 | 默认值 | 备注             |
    |----------------+------+---------+-------------------|
    | leakResistance |      |         | *[新特性 v99 (268)]*泄露抵抗度 |
...
*** 装甲导管
    *[新特性 v99 (268)]* -- 继承 [[导管]]，英文:ArmoredConduit
...
*** 液体溢流门
    *[新特性 v99 (268)]* -- 继承 [[液体方块]]，英文:LiquidOverflowGate
...
**** 地热发电机
    继承 [[发电机][发电机]] -- 用 [[属性][属性]] 的地板发电. 输出公式是 ~powerProduction * 属性~, 和 ~属性~ 必须大于 ~0.01~.英文：ThermalGenerator

    | 接口          | 类型   | 默认值 | 备注      |
    |----------------+--------+---------+------------|
    |                |        |         | <10>       |
    | generateEffect | [[特效][特效]] | none    |   发电特效         |
	| attribute      | [[属性][属性]] | heat    | 提升效率的属性. |

**** 物品液体发电机
    继承 [[发电机][发电机]] -- 基础发电机.英文：ItemLiquidGenerator
...
**** 新核电站
    ...
    |-----------------+-------+----------+------------|
    |                 |       |          | <10>       |
    | lightColor      | [[颜色][颜色]] |   7f19ea | *[new v99 (268)]* |
...
*** 灯
    *[新特性 v99 (268)]* -- 继承 [[方块]]，英文：LightBlock

    | 接口      | 类型  | 默认值 | 备注 |
    |------------+-------+--------+-|
    | brightness | float |     0.9 | 亮度 |
    | radius     | float |     200 | 范围 |
...
** 生产工厂
   ...
   :END:
*** 钻头
    继承 [[方块][方块]] -- 一组能从 [[矿物][矿物]] 获取物品的方块.英文：Drill
...
*** 存储方块
    ...
    [[Abstract][Abstract]] 修饰类继承 [[方块][方块]]，英文：StorageBlock
...
*** 卸载器
    ...
    继承 [[方块][方块]]
    一个方块可以从 [[存储方块][存储方块]], 比如 [[仓库][仓库]], [[核心][核心]] 或 [[工厂][工厂]] 中拿物品.英文：Unloader
...
** 物品
   ...
   |----------------+----------+---------+------------|
   |                |          |         | <10>       |
   | color          | [[颜色][颜色]]    |   black      | 分类器显示色 |
   | type           | [[物品][物品]] | resource        | ~resource~ 或是 ~material~ ; resource不能进核心，反之 |
...
** 物品组
   ...
   *[新特性 968f3ace3]:* 一个 ~ItemStack~ 可以用这个格式 ~"item/amount"~.
   
   一个 ~ItemStack~ 要包括以下接口:
   ...
   你经常看到 ~[ ItemStack ]~, 这个代表 ~ItemStack~ 的组, 比如:
...
** 液体
   ...
   定义液体的东西. 像 [[物品]] 一样，这个内容物有自己的子文件夹 ~content/liquids/liquid-name.json~, 这个液体名字可以在其他内容物重复使用.

   | 接口         | 类型         | 默认值 | 备注      |
   |---------------+--------------+---------+------------|
   |               |              |     <r> | <10>       |
   | color         | [[颜色][颜色]]        |         | *[必须]* 液体颜色 |
   | barColor      | [[颜色][颜色]]        |         | /[非必须]/ UI中颜色. |
   | lightColor    | [[颜色][颜色]]        |         | 发光颜色.改变光度不在这里. |
...
** 液体组
   ...
   *[新特性 968f3ace3]:* 一个 ~LiquidStack~ 可以用这个格式 ~"liquid/amount"~.

    一个 ~LiquidStack~ 要有一下接口:
...
** 武器
   ...
   | targetDistance | float      |       1 | 如果 ~turnCursor~ 是 ~false~ 在一个玩家机甲, 这个武器会瞄准多远. |
...
** 单位类
   ...
   | toMine          | [[物品][物品]] | copper,lead   | 挖的东西 |
   | deathSound      | [[音效][音效]] | bang | 死亡音效 |
...
** 玩家机甲
   ...
   | drawCell           | boolean |    true | 是否渲染队伍灯            |
   | drawItems          | boolean |    true | 是否渲染所持物品                    |
   | drawLight          | boolean |    true | 是否渲染引擎 |
...
** 区域
   ...
   |----------------------+---------------+-------------+------------|
   |                      |               |             | <10>       |
   | baseLaunchCost       | [[物品组][物品组]]     |             |    基础发射消耗资源        |
   | launchCost           | [[物品组][物品组]]     |             |    发射消耗资源        |
   | startingItems        | [[物品组][物品组]]     |             | 初始物品. |
   | conditionWave        | int           | MAX_VALUE   |  总波数          |
   | alwaysUnlocked       | boolean       | false       | 地图是否一直解锁 |
   | launchPeriod         | int           | 10          | 核心发射间隔波次. |
   | loadout              | [[蓝图][蓝图]]     | basicShard  | 初始核心. |
   | resources            | [ String ]    |             | 一个 [[物品][物品]] 名字的组. |
   | requirements         | [ [[元素][元素]] ] |             | 一组开启地图的条件. |
   | configureObjective   | [[元素][元素]]     | ZoneWave 15 |  可自定义发射物品波次          |
   | defaultStartingItems | [[物品组][物品组]]     |             |  默认初始物品          |
...
** 状态
   ...
   /不要和 [[特效][特效]] 搞混/,状态会给一个实体额外属性. -- 状态有时会充当效果的两个条件. 比如一个 ~wet~ 的单位获得了 ~shocked~ 此时它会掉20血.
...
** 子弹类
   ...
   [[Abstract][Abstract]] 修饰类继承 [[内容][内容]]

   子弹类可以是一个元素 ~{}~ 或一个 ~"字符串"~, 字符串就是调用 [[内建子弹]] 元素就是自定义子弹.
   ...
   * 其他特殊的子弹类.
   ...
   | fragVelocityMin    | float        |     0.2 | 破片最小速度倍率. |
   | fragVelocityMax    | float        |       1 | 破片最大速度倍率. |
   | fragBullet         | [[子弹类][子弹类]]   |    null | 破片弹，null来取消生成. |
   | splashDamage       | float        |       0 | 范围伤害. 用 [[https://en.wikipedia.org/wiki/Linear_interpolation][linear interpolation]] 计算, 也是熟悉的 lerp 函数. |
   | splashDamageRadius | float        |      -1 | 范围伤害范围. ~splashDamageRadius~ 这个值与上一个配合作用于函数 ~lerp(1 - distance / radius, 1, 0.4)~ 是 ~splashDamage~ 的倍率. |
...
**** 炮塔弹
     ...
     :DESCRIPTION: 下雨一般的特效
     :END:
     继承 [[基础子弹][基础子弹]]，英文：ArtilleryBulletType
...
**** 破片弹
     ...
     :DESCRIPTION: 融合在一起
     :END:
     继承 [[基础子弹][基础子弹]]，英文：FlakBulletType
...
**** 导弹
     ...
     :DESCRIPTION: 特效和尾迹
     :END:
     继承 [[基础子弹][基础子弹]]，英文：MissileBulletType
...
**** 炸弹
     ...
     继承 [[基础子弹][基础子弹]]，英文：BombBulletType
...
*** 治疗子弹
    ...
    继承 [[子弹类][子弹类]] -- 可以治疗同队方块和单位的子弹.英文：HealBulletType
...
*** 液体子弹
    ...
    继承 [[子弹类][子弹类]]，英文：LiquidBulletType

    | 接口  | 类型   | 默认值 | 备注      |
    |--------+--------+---------+------------|
    |        |        |         | <10>       |
    | liquid | String | null    | *[必须]* [[液体][液体]]的名字 |
...
* 元素
  ...
  元素是某个参数的特征,有几个类引用它,用于 [[区域][区域]] 地图设置中的元素设定. 

  使用元素的类:
  * ~ZoneWave~ -- 当选定 ~区域~ 打到指定 ~波次~ 触发
    | 接口 | 类型   | 备注                |
    |-------+--------+----------------------|
    | zone  | String | 目标 [[区域][区域]] 名字     |
    | wave  | int    | 目标波次 |
